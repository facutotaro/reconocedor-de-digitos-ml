...
unsigned long KNNClassifier::squareDistance(Vector v1, Vector v2)
{
    long res = 0;
...
Vector KNNClassifier::bestK(Vector v)
{
    ...
    {
        //En caso de que aparezca un nuevo con menor distancia que el mas lejano de los vecinos, lo inserto(uso insertion porque para listas ordenadas es el
        //menos costoso)
        unsigned long distance = squareDistance(v, digits_.row(i));
        if (distance < dist(n_neighbors_ - 1)) //Que hacemos si se empata en el ultimo puesto?
        {
